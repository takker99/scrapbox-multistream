...
import { ModifiedBlocks, ParsedLine } from "./parse.ts";

JSXSlack.exactMode(true);

export function sb2blocks({ project, title, chunks }: ModifiedBlocks) {
  return chunks.map((blocks, i) => (
    <>
      {blocks.map((block) =>
        <Block project={project} title={title} block={block} />
      )}
      {i !== chunks.length && <br />}
    </>
  ));
...
const Block = ({ project, title, block }: BlockProps) => {
  switch (block.type) {
    case "title":
      return <>{block.text}</>;
    case "codeBlock":
...
const CodeBlock = (
  { project, title, fileName, text }: CodeBlockProps,
) => (
  <Section>
    <a
...
function Node(
  ...
  switch (node.type) {
    ...
    case "hashTag":
      return (
        <a href={`https://scrapbox.io/${project}/${node.href}`}>
          #{node.href}
        </a>
      );
    case "plain":
...
const Decoration = ({ decos, nodes, project, title }: DecorationProps) => {
  ...
  let renderedNodes = (
    <>
      {nodes.map((node) =>
        <Node node={node} project={project} title={title} />
      )}
    </>
  );
